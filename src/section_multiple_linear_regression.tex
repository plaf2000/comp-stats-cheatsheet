...
A sequence of regression function estimates $\{\hat{f_n}\}$ is called weakly universally consistent if it is weakly consistent, i.e. $\lim_{n \to \infty}{\mathbb{E}\left[\int_{\mathbbR^p}{(\hat{f}_n(x) - f^{*}(x)}^2P(dx)\right]} = 0$.
